...
// Define MAX_NEW_TOKENS if it's not already defined
const MAX_NEW_TOKENS = 500;

// Add a lock mechanism to prevent multiple simultaneous processing
...
async function generate(messages) {
  ...
  if (isProcessing) {
    ...
    self.postMessage({
      status: "error",
      data: "Already processing a request, please try again later",
    });
    return;
  }
...
// Simple check function to verify the worker is running
function check() {
  self.postMessage({ status: "check", data: "Worker is running" });
}
